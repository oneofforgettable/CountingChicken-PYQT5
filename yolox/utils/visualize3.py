...
def plot_tracking_vote(total_id_top_1, total_id_bottom_1, total_id_left_1, total_id_right_1,
        ...
        if obj_ids is not None:
            ...
            if count_center > 1:
                for k in range(count_center - 1):
                    cv2.line(im, (center[k * 2], center[k * 2 + 1]), (center[(k + 1) * 2], center[(k + 1) * 2 + 1]),
    ...
    cv2.putText(im, 'frame: %d total_id: %d current_num: %d middle_num: %d avg_num: %d weight_num: %d max_num: %d'
                % (frame_id, len(center_points.keys()), len(tlwhs), chicken_number[0], chicken_number[1],
...
def plot_tracking_vote_direction(total_id_top_1, total_id_bottom_1, total_id_left_1, total_id_right_1,
                                 total_id_top_2, total_id_bottom_2, total_id_left_2, total_id_right_2,
                                 total_id_top_3, total_id_bottom_3, total_id_left_3, total_id_right_3,
                                 center_points, direction, image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0.,
                                 ids2=None):
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]
    top_view = np.zeros([im_w, im_w, 3], dtype=np.uint8) + 255
...
def cv2AddChineseText(img, text, position, textColor=(0, 255, 255), textSize=50):
    ...
    font_style = ImageFont.truetype(
        "/home/chuanzhi/mnt_2T/Wu/CountingChicken/yolox/utils/font/STKAITI.TTF", textSize, encoding="utf-8")
    # 绘制文本
...
def plot_wutracking_vote(total_count, center_points, direction, image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0.,
                         ids2=None):
    im = np.ascontiguousarray(np.copy(image))
    ...
    top_view = np.zeros([im_w, im_w, 3], dtype=np.uint8) + 255
    text_scale = max(3, image.shape[1] / 900.)
    text_thickness = 2
    line_thickness = max(1, int(image.shape[1] / 500.))
    radius = max(5, int(im_w / 140.))
    ...
    points_Line = np.array(
        [
            [line_x - line_x * 0.05, int(im.shape[0] * 0.2) + 1],
            [line_x + line_x * 0.05, int(im.shape[0] * 0.2) + 1],
            [line_x + line_x * 0.05, int(im_h) + int(im.shape[0] * 0.2) - 1],
            [line_x - line_x * 0.05, int(im_h) + int(im.shape[0] * 0.2) - 1]
        ], dtype=np.int32)

    # 将顶点坐标调整为适当的形状
    points_Line = points_Line.reshape((-1, 1, 2))

    #im = cv2.copyMakeBorder(im, int(im.shape[0] * 0.2), 0, 0, 0, cv2.BORDER_ISOLATED, value=[0, 0, 0])

    zeros = np.zeros(im.shape, dtype=np.uint8)

    mask_Line = cv2.fillPoly(zeros, [points_Line], color=(98, 9, 11))

    rate = 0.5  # 透明度
    im = rate * mask_Line + im
    ...
    im = cv2AddChineseText(im, '华南农业大学 智能数鸡', (int(1), int(4 * text_scale)), (255, 0, 0), 50)
    im = cv2AddChineseText(im, '总数量: %d ' % (len(total_count)), (int(1), int(22 * text_scale)), (255, 0, 0), 50)

    # cv2.putText(im, '总数量: %d '
    #            % (len(total_count)) , (int(1), int(15 * text_scale)) , cv2.FONT_HERSHEY_PLAIN , text_scale , (0, 0, 255), thickness=2, fontProperties=font_prop)
    cv2.imwrite(r"./tmp.jpg", im)
    return im, total_count, center_points


####################################### 使用Wu Tracker跟踪器   只记ID线数目 ################################################
def plot_wutracking_id(total_count, center_points, direction, image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0.,
                       ids2=None):
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]
    ...
    ## 添加图片的标签
    cv2.putText(im, 'frame: %d total_id: %d current_num: %d total_num: %d '
                % (frame_id, len(center_points.keys()), len(tlwhs), len(total_count)), (0, int(15 * text_scale)),
                cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255), thickness=2)
    return im, total_count, center_points


####################################### 使用Wu Tracker跟踪器   只记line线数目 ################################################
def plot_wutracking_line(total_count, center_points, direction, image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0.,
                         ids2=None):
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]
...
def plot_wutracking_region(total_count, center_points, direction, image, tlwhs, obj_ids, scores=None, frame_id=0,
                           fps=0., ids2=None):
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]
