...
def cv2AddChineseText(img, text, position, textColor=(0, 255, 255), textSize=50):
    ...
    fontStyle = ImageFont.truetype(
        "./font/STKAITI.TTF", textSize, encoding="utf-8")
    # 绘制文本
...
def plot_wutracking_vote(total_count, center_points, direction, image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
    ...
    # 定义多边形的顶点坐标
    points_Line = np.array([[line_x-line_x*0.05, 1], [line_x+line_x*0.05, 1], [line_x+line_x*0.05, int(im_h) - 1],[line_x-line_x*0.05, int(im_h) - 1] ], dtype=np.int32)
    points_count = np.array([[1, int(line_y*0.1)], [int(line_x * 0.4), int(line_y*0.1)], [int(line_x * 0.4), int(line_y*0.3)],[1, int(line_y*0.3)] ], dtype=np.int32)
    ...
    mask_count = cv2.fillPoly(zeros, [points_count], color=(98, 9, 11) )

    rate = 0.5 # 透明度
    im = rate*mask_Line + im + mask_count
    ...
    # im = cv2AddChineseText(im, '总数量: %d '
    #             % (len(total_count)) , (int(1), int(15 * text_scale)), (255, 0, 0), 50)

    cv2.putText(im, 'Total: %d '
