...
def make_parser():
    ...
    parser.add_argument(
        ...
        "--path",
        default="/media/z/ae609a98-67c3-41ce-beed-791c5c3bf738/zzy/data/8月28日小鸡视频/麻羽2/VID_20210828_161118.mp4",
    ...
    parser.add_argument("--model_folder", type=str, default='pretrained/deepsort_reid_ckpt.t7', help="reid model folder")
    return parser
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    # 使用Sort
    tracker = DeepSort(args.model_folder)
    timer = Timer()
    frame_id = 0
    results = []
    ...
    while True:
        ...
        if ret_val:
            ...
            for t in online_targets:
                # tlwh = t.tlwh
                # tid = t.track_id
                tlwh = [t[0], t[1], abs(t[2]-t[0]), abs(t[3]-t[1])]
                tid = t[4]
                # vertical = tlwh[2] / tlwh[3] > 1.6
                # if tlwh[2] * tlwh[3] > args.min_box_area and not vertical:
...
def write_count_chicken(save_folder, video, count, id_count, fps):
    ...
    result_str = '{},{},{}\n'.format(video_name, count, fps)
    if not os.path.exists(result_fpath):
        with open(result_fpath, 'a') as f:
            f.write('video_name,count,fps\n')
            f.write(result_str)
    else :
        with open(result_fpath, 'a') as f:
            f.write(result_str)
